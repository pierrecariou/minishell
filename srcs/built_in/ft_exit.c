...
/*   Updated: 2020/11/24 14:51:27 by pcariou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static int	ft_alpha_error(char *argv, int i)
{
...
static int	ft_cal_output(char *argv)
{
	...
	while (argv[++i])
	{
		if ((ret == 922337203685477580 && argv[i] > '7') ||
			(ret > 922337203685477580 && argv[i]))
		{
			if (minus == 1 && ret == 922337203685477580
				&& argv[i] > '7')
				return (ft_alpha_error(argv, 0));
			return (ft_alpha_error(argv, 2));
		}
		ret *= 10;
		ret += (argv[i] - '0');
	}
...
static int	ft_grezette_exit(t_cmd cmd, t_cmdv *cmdv)
{
	...
	if (i != ft_strlen(cmd.argv[1]))
		return (ft_alpha_error(cmd.argv[1], 2));
...
int			ft_exit(t_cmd cmd, t_cmdv *cmdv, char *file, char **paths)
{
	...
	cmdv->err_built = (i == 2) ? 2 : cmdv->err_built;
	if (i == 257)
	{
		cmdv->err_built = 1;
		return (1);
